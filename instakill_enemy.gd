...
func _physics_process(delta: float) -> void:
	...
	var vector_flanqueado = Vector2( -direccion.y, direccion.x)
	...
	match currnt_sts:
		sts.SEARCH:
			if distancia >= 332.00:
				...
				velocity = direccion * alt_velocidad
				
			if distancia > distancia_ataque and distancia < 337:
				velocity = vector_flanqueado * velocidad
