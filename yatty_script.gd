...
func _physics_process(delta: float) -> void:
	...
	if distancia > distancia_ataque:
		...
		# Cerca: velocidad reducida (NO cero) + disparar
		velocity.y = direccion.y * velocidad_cerca
		intentar_disparar(direccion)
	
	# 4. Limitar el rango de movimiento en Y
	currnt_pos_y = global_position.y
	global_position.y = clamp(currnt_pos_y, start_pos - max_range_y, start_pos + max_range_y)
